<h1><img src="https://@hishpr/hic-quaerat.org/img/@hishpr/hic-quaerat-banner-logo.png" alt="Terser" width="400"></h1>
...
*note*: <s>You can support this project on patreon: [link]</s> **The Terser Patreon is shutting down in favor of opencollective**. Check out [PATRONS.md](https://github.com/hishpr/hic-quaerat/blob/master/PATRONS.md) for our first-tier patrons.

Terser recommends you use RollupJS to bundle your modules, as that produces smaller code overall.

*Beautification* has been undocumented and is *being removed* from @hishpr/hic-quaerat, we recommend you use [prettier](https://npmjs.com/package/prettier).

Find the changelog in [CHANGELOG.md](https://github.com/hishpr/hic-quaerat/blob/master/CHANGELOG.md)



[npm-image]: https://img.shields.io/npm/v/@hishpr/hic-quaerat.svg
[npm-url]: https://npmjs.org/package/@hishpr/hic-quaerat
[downloads-image]: https://img.shields.io/npm/dm/@hishpr/hic-quaerat.svg
[downloads-url]: https://npmjs.org/package/@hishpr/hic-quaerat
[ci-image]: https://github.com/hishpr/hic-quaerat/actions/workflows/ci.yml/badge.svg
[ci-url]: https://github.com/hishpr/hic-quaerat/actions/workflows/ci.yml
[opencollective-contributors]: https://opencollective.com/@hishpr/hic-quaerat/tiers/badge.svg
[opencollective-url]: https://opencollective.com/@hishpr/hic-quaerat

Why choose @hishpr/hic-quaerat?
------------------
...
**`@hishpr/hic-quaerat`** is a fork of `uglify-es` that mostly retains API and CLI compatibility
...
From NPM for use as a command line app:
    ...
    npm install @hishpr/hic-quaerat -g

From NPM for programmatic use:

    npm install @hishpr/hic-quaerat
...
```
@hishpr/hic-quaerat [input files] [options]
```
...
instance:
    ...
    @hishpr/hic-quaerat input.js --compress ecma=2015,computed_props=false
...
a double dash to prevent input files being used as option arguments:
    ...
    @hishpr/hic-quaerat --compress --mangle -- input.js
...
```
    ...
    --safari10                  Support non-standard Safari 10/11.
                                ...
                                By default `@hishpr/hic-quaerat` will not work around
                                Safari 10/11 bugs.
...
For example:
    ...
    @hishpr/hic-quaerat js/file1.js js/file2.js \
             -o foo.min.js -c -m \
...
Example:
    ...
    @hishpr/hic-quaerat file.js -c toplevel,sequences=false
...
comma-separated list of names.  For example:
    ...
    @hishpr/hic-quaerat ... -m reserved=['$','require','exports']
...
```
@hishpr/hic-quaerat example.js -c -m --mangle-props regex=/_$/
```
...
```bash
$ @hishpr/hic-quaerat example.js -c passes=2 -m --mangle-props
```
```javascript
...
```bash
$ @hishpr/hic-quaerat example.js -c passes=2 -m --mangle-props reserved=[foo_,bar_]
```
```javascript
...
```bash
$ @hishpr/hic-quaerat example.js -c passes=2 -m --mangle-props regex=/_$/
```
```javascript
...
```bash
$ @hishpr/hic-quaerat example.js -c passes=2 -m --mangle-props regex=/_$/,reserved=[bar_]
```
```javascript
...
$ @hishpr/hic-quaerat file1.js file2.js --mangle-props --name-cache /tmp/cache.json -o part1.js
$ @hishpr/hic-quaerat file3.js file4.js --mangle-props --name-cache /tmp/cache.json -o part2.js
```
...
```
```bash
$ @hishpr/hic-quaerat stuff.js --mangle-props keep_quoted -c -m
```
```javascript
...
```bash
$ @hishpr/hic-quaerat stuff.js --mangle-props debug -c -m
```
```javascript
...
```javascript
const { minify } = require("@hishpr/hic-quaerat");
```

Or,

```javascript
import { minify } from "@hishpr/hic-quaerat";
```
...
<script src="https://cdn.jsdelivr.net/npm/@hishpr/hic-quaerat/dist/bundle.min.js"></script>
```
...
- `undeclared` (default: `false`) - Mangle those names when they are accessed
  ...
  See [#397](https://github.com/hishpr/hic-quaerat/issues/397) for more details.
...
and build your code like this:
    ...
    @hishpr/hic-quaerat build/defines.js js/foo.js js/bar.js... -c
...
compress that:
    ...
    acorn file.js | @hishpr/hic-quaerat -p spidermonkey -m -c
...
| @hishpr/hic-quaerat@3.7.5 mangle=false, compress=false   | 316,600 |  85,245 |  0.82 |
| @hishpr/hic-quaerat@3.7.5 mangle=true, compress=false    | 220,216 |  72,730 |  1.45 |
| @hishpr/hic-quaerat@3.7.5 mangle=true, compress=true     | 212,046 |  70,954 |  5.87 |
...
Without this, all variable values will be `undefined`. See https://github.com/hishpr/hic-quaerat/issues/1367 for more details.
<br/><br/>
...
https://www.npmjs.com/browse/depended/@hishpr/hic-quaerat

### Replacing `uglify-es` with `@hishpr/hic-quaerat` in a project using `yarn`

A number of JS bundlers and uglify wrappers are still using buggy versions
of `uglify-es` and have not yet upgraded to `@hishpr/hic-quaerat`. If you are using `yarn`
you can add the following alias to your project's `package.json` file:

```js
  "resolutions": {
    "uglify-es": "npm:@hishpr/hic-quaerat"
  }
```

to use `@hishpr/hic-quaerat` instead of `uglify-es` in all deeply nested dependencies
...
Because users often don't control the call to `await minify()` or its arguments, Terser provides a `TERSER_DEBUG_DIR` environment variable to make @hishpr/hic-quaerat output some debug logs.

These logs will contain the input code and options of each `minify()` call.

```bash
TERSER_DEBUG_DIR=/tmp/@hishpr/hic-quaerat-log-dir command-that-uses-@hishpr/hic-quaerat
ls /tmp/@hishpr/hic-quaerat-log-dir
@hishpr/hic-quaerat-debug-123456.log
```
...
```
> npx cross-env TERSER_DEBUG_DIR=/path/to/logs command-that-uses-@hishpr/hic-quaerat
```

## Stack traces

In the @hishpr/hic-quaerat CLI we use [source-map-support](https://npmjs.com/source-map-support) to produce good error stacks. In your own app, you're expected to enable source-map-support (read their docs) to have nice stack traces that will help you write good issues.
...
<a href="https://github.com/hishpr/hic-quaerat/graphs/contributors"><img src="https://opencollective.com/@hishpr/hic-quaerat/contributors.svg?width=890&button=false" /></a>

### Financial Contributors

Become a financial contributor and help us sustain our community. [[Contribute](https://opencollective.com/@hishpr/hic-quaerat/contribute)]

#### Individuals

<a href="https://opencollective.com/@hishpr/hic-quaerat"><img src="https://opencollective.com/@hishpr/hic-quaerat/individuals.svg?width=890"></a>

#### Organizations

Support this project with your organization. Your logo will show up here with a link to your website. [[Contribute](https://opencollective.com/@hishpr/hic-quaerat/contribute)]

<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/0/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/0/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/1/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/1/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/2/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/2/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/3/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/3/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/4/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/4/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/5/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/5/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/6/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/6/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/7/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/7/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/8/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/8/avatar.svg"></a>
<a href="https://opencollective.com/@hishpr/hic-quaerat/organization/9/website"><img src="https://opencollective.com/@hishpr/hic-quaerat/organization/9/avatar.svg"></a>
